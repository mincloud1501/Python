...
exports.parseInstancedModel3DTileSync = parseInstancedModel3DTileSync;
...
var _math2 = require("@loaders.gl/math");
...
function parseInstancedModel(tile, arrayBuffer, byteOffset, options, context) {
  ...
  tile.rtcCenter = featureTable.getGlobalProperty('RTC_CENTER', _math2.GL.FLOAT, 3);
...
function extractInstancedAttributes(tile, featureTable, batchTable, instancesLength) {
  ...
  var instancePosition = new _math.Vector3();
  var instanceNormalRight = new _math.Vector3();
  var instanceNormalUp = new _math.Vector3();
  var instanceNormalForward = new _math.Vector3();
  var instanceRotation = new _math.Matrix3();
  var instanceQuaternion = new _math.Quaternion();
  var instanceScale = new _math.Vector3();
  var instanceTranslationRotationScale = {};
  var instanceTransform = new _math.Matrix4();
  var scratch1 = [];
  var scratch2 = [];
  ...
  for (var i = 0; i < instancesLength; i++) {
    ...
    if (featureTable.hasProperty('POSITION')) {
      position = featureTable.getProperty('POSITION', _math2.GL.FLOAT, 3, i, instancePosition);
    } else if (featureTable.hasProperty('POSITION_QUANTIZED')) {
      position = featureTable.getProperty('POSITION_QUANTIZED', _math2.GL.UNSIGNED_SHORT, 3, i, instancePosition);
      var quantizedVolumeOffset = featureTable.getGlobalProperty('QUANTIZED_VOLUME_OFFSET', _math2.GL.FLOAT, 3, scratchVector1);

      if (!quantizedVolumeOffset) {
        throw new Error('i3dm parser: QUANTIZED_VOLUME_OFFSET must be defined for quantized positions.');
      }

      var quantizedVolumeScale = featureTable.getGlobalProperty('QUANTIZED_VOLUME_SCALE', _math2.GL.FLOAT, 3, scratchVector2);
    ...
    tile.normalUp = featureTable.getProperty('NORMAL_UP', _math2.GL.FLOAT, 3, i, scratch1);
    tile.normalRight = featureTable.getProperty('NORMAL_RIGHT', _math2.GL.FLOAT, 3, i, scratch2);
    ...
    if (tile.normalUp) {
      ...
      tile.octNormalUp = featureTable.getProperty('NORMAL_UP_OCT32P', _math2.GL.UNSIGNED_SHORT, 2, scratch1);
      tile.octNormalRight = featureTable.getProperty('NORMAL_RIGHT_OCT32P', _math2.GL.UNSIGNED_SHORT, 2, scratch2);
    ...
    var scale = featureTable.getProperty('SCALE', _math2.GL.FLOAT, 1, i);

    if (Number.isFinite(scale)) {
      instanceScale.multiplyByScalar(scale);
    }

    var nonUniformScale = featureTable.getProperty('SCALE_NON_UNIFORM', _math2.GL.FLOAT, 3, i, scratch1);
    ...
    var batchId = featureTable.getProperty('BATCH_ID', _math2.GL.UNSIGNED_SHORT, 1, i);

    if (batchId === undefined) {
      batchId = i;
    }

    var rotationMatrix = new _math.Matrix4().fromQuaternion(instanceTranslationRotationScale.rotation);
